...
public class EngineSnifferActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (finalUserAgent == null) {
            // Only force Chrome/WebView UA if NOT using GeckoView (GeckoView should use its
            // own Firefox UA)
        ...
        if (targetUrl.contains("|Referer=")) {
            ...
            Log.d(TAG, "=== REFERER EXTRACTION ===");
            Log.d(TAG, "Clean URL: " + cleanUrl);
            Log.d(TAG, "Extracted Referer: " + referer);
            Log.d(TAG, "Headers map: " + headers);
        } else {
            Log.d(TAG, "No embedded Referer found in URL");
        }

        // Pass cookies from CookieManager (critical for GeckoView which has isolated
        // session)
        String cookies = CookieManager.getInstance().getCookie(cleanUrl);
        if (cookies != null) {
            headers.put("Cookie", cookies);
            Log.d(TAG, "Added cookies to headers: " + cookies);
        }
        ...
        // CRITICAL: Restore cookies from DB before loading (same pattern as
        // SnifferActivity)
        // This ensures CF clearance cookies are in CookieManager before GeckoView
        // accesses them
        ...
        scraperManager.restoreCookiesForUrl(finalCleanUrl, () -> {
            ...
            if (!finalHeaders.isEmpty()) {
                ...
                Log.d(TAG, "Calling engine.loadUrl(finalCleanUrl, finalHeaders)");
                engine.loadUrl(finalCleanUrl, finalHeaders);
            } else {
    ...
    private void injectVideoDetectionScript() {
        ...
        String script = "(function() {" +
                ...
                "    }" +
                "  }" +
                // Also check jwplayer if available
                "  if (typeof jwplayer !== 'undefined') {" +
                "    try {" +
                "      var player = jwplayer();" +
